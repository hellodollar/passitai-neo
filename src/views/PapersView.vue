...
import { PAPER_CATEGORY_LABELS } from '@/constants/app'
import { useAppStore } from '@/stores/app'
import { useLearningStore } from '@/stores/learning'
import type { PaperCategory } from '@/types/domain'

const app = useAppStore()
const learning = useLearningStore()

const searchQuery = ref('')
const categoryFilter = ref<PaperCategory | 'all'>('all')
...
const filteredPapers = computed(() => {
  ...
  return learning.papers.filter((paper) => {
    ...
    const matchesCategory = categoryFilter.value === 'all' || paper.paperCategory === categoryFilter.value
    ...
    <div class="rounded-2xl border border-base-200 bg-base-100 p-3">
      <div class="grid gap-3 lg:grid-cols-[1fr_14rem]">
        <label class="input input-bordered flex h-10 items-center gap-2 rounded-2xl">
          ...
          <input v-model="searchQuery" type="search" class="grow text-sm" placeholder="搜索试卷名称" />
        </label>
        <select v-model="categoryFilter" class="select select-bordered h-10 w-full rounded-2xl text-sm">
          <option value="all">全部类型</option>
          <option v-for="(label, value) in PAPER_CATEGORY_LABELS" :key="value" :value="value">{{ label }}</option>
        </select>
      </div>
    </div>
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="min-w-0">
              ...
              <div class="mt-1 flex flex-wrap gap-1.5">
                <span class="badge badge-outline badge-sm">{{ PAPER_CATEGORY_LABELS[paper.paperCategory] }}</span>
                <span class="badge badge-ghost badge-sm">{{ paper.createdBy }}</span>
              </div>
            </div>
          </div>
          <RouterLink class="btn btn-primary btn-sm rounded-full" :to="`/practice?paperId=${paper.id}`">
            开始
          </RouterLink>
        </div>
      </article>
